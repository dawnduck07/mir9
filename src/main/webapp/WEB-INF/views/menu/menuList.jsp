...
<script>
        ...
        function onclickRevision(code) {
            ...
            $.ajax({
                url:'/mir9/menu/json/getRevisionList?${_csrf.parameterName}=${_csrf.token}',
                type:'post',
                dataType:'json',
                data:{
        ...
        function onclickRevision2() {
            parent.$('#modalRevision').modal({backdrop:'static', show:true});
            $.ajax({
                url:'/mir9/menu/json/deleteRevisionList?${_csrf.parameterName}=${_csrf.token}',
                type:'post',
                dataType:'json',
                data:{
        ...
        function updateChoiceMenu(code){
			...
			if(!confirm("해당 자료를 정말 삭제 하시겠습니까?")){
	  		...
	  		$.ajax({
  			 	 url : "/mir9/menu/updateChoiceMenu?${_csrf.parameterName}=${_csrf.token}",
	  		  	 type : "POST",
  		  	 	 data : { 
  		  	 		menuArr : menuArr 
  		  	 	 },
